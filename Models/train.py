...
import pandas as pd
import os
from evaluation import results2CSV_train
...
"""We make changes on Attention_based MIL (Copyright © 2018 ilse.maximilian@gmail.com.) to train viral protein embeddings.
"""
...
if __name__ == "__main__":
    ...
    for i in [4]:
        hostname=virus_host.iloc[i,0]
        for j in range(5):
            print('Start Training')
            max_acc = 0                
            train_loader=torch.load(input_data_path + hostname +'_train_dl_'+str(j))
            val_loader=torch.load(input_data_path + hostname +'_val_dl_'+str(j))
            for epoch in range(1, args.epochs + 1):
                ...
                if epoch % snapStep == 0 or epoch >= args.epochs:
                    ...
                    if max_acc <= val_acc:
                        max_acc = val_acc
                        model_save_path = os.path.join(output+ hostname +'_model.'+str(j+1))
