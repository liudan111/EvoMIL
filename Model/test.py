...
if __name__ == "__main__":
    ...
    virus_host_30 = pd.read_csv(input+'final_specise_sort_30_pro.csv') #Eukaryota_final_specise_sort_30
    length=virus_host_30.shape[0]
    for m in range(length):
        hostname=virus_host_30.iloc[m,0]
        test_loader=torch.load(input_data_path+'test_loader_'+str(m+1))
        for j in range(5):
